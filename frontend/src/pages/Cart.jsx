  ...
  for(const productId in cartItems) {
    for(const size in cartItems[productId]) {
      ...
      if(cartItems[productId][size] > 0) {
        cartData.push({ _id: productId, size: size, quantity: cartItems[productId][size] });
      }
    } 
  }
      ...
      <div className="flex justify-end my-20">
        <div className="w-full sm:w-[450px]">
          ...
          <div className="w-full text-end">
            <button onClick={() => navigate("/place-order")} className="bg-black text-white text-sm px-6 py-3 my-8">PROCEED TO CHECKOUT</button>
          </div>
        </div>
      </div>  
